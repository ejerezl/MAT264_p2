...
In the following the approximations of the Lax-Wendroff method will be presented in comparison to Lax-Friedrich and Godunov on a Riemann problem.
...
As commented before the Lax-Wendroff scheme oscillates in front of the discontinuity of the true solution. But on the other hand the discontinuity is approximated better what leads to a smaller error which is shown in \cref{img:error_over_time}. Also notice that the Lax-Friedrich does not reach the 
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Approximations for the advection equation with different flows after some time.}
\end{figure}
